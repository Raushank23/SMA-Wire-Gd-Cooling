...
# Equilibrium temperature calculation with combined radiation and convection
def equilibrium_temperature(T_SMA_initial, T_Gadolinium_initial, m_SMA, c_SMA, m_Gadolinium, c_Gadolinium, h_SMA, h_Gadolinium):
    T_eq = (m_SMA * c_SMA * T_SMA_initial + m_Gadolinium * c_Gadolinium * T_Gadolinium_initial +
            (m_SMA * h_SMA + m_Gadolinium * h_Gadolinium) * (T_air + 273.15)) / (m_SMA * c_SMA + m_Gadolinium * c_Gadolinium + m_SMA * h_SMA + m_Gadolinium * h_Gadolinium)
    return T_eq

T_eq = equilibrium_temperature(T_SMA_initial, T_Gadolinium_initial, m_SMA, c_SMA, m_Gadolinium, c_Gadolinium, h_SMA, h_Gadolinium)
...
data = {
    'Time (s)': sol.t,
    'SMA Wire Temperature (°C)': sol.y[0] - 273.15,
    'Gadolinium Tube Temperature (°C)': sol.y[1] - 273.15
}
df = pd.DataFrame(data)

# Save to an Excel file
df.to_excel('temperature_vs_time.xlsx', index=False)

# Save to a CSV file
df.to_csv('temperature_vs_time.csv', index=False)
...
plt.axhline(y=T_eq_Celsius, color='r', linestyle='--', label=f'Equilibrium Temperature: {T_eq_Celsius:.2f} °C')
